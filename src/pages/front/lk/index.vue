<template>
  <q-page>
    <div class="container">
      <p class="text-h2">Личный кабинет</p>
      <q-tabs
        v-model="tab"
        dense
        class="text-grey"
        ...
        narrow-indicator
      >
        <q-tab name="orders" label="Мои заказы" />
          ...
          >
            ...
            <template v-slot:body="props">
              ...
              <q-tr v-show="props.expand" :props="props">
                <q-td colspan="100%">
                  <div class="text-left">Не хватает: {{ props.row.missing_info }}.</div>
                </q-td>
              </q-tr>
            </template>
...
const columns = [
  { name: 'id', align: 'left',  label: 'ID Заказа', field: 'id',  sortable: true, is_link:false},
  { name: 'service_name', align: 'left',  label: 'Услуга', field: row => row.service.name ,  sortable: true, is_link:false},
  { name: 'order_status', align: 'left',  label: 'Статус заказа', field: row => row.order_status.name ,  sortable: true, is_link:false},
  { name: 'pay_status', align: 'left',  label: 'Статус оплаты', field: row => row.pay_status.name ,  sortable: true, is_link:false},
...
onMounted(async ()=>{
  ...
  await getOrders()
  user.value = auth_store.user
  user_files.value = auth_store.user.files
  user_networks.value = auth_store.user.networks
  loading.value = !loading.value
})
...
const getOrders = async () => {
  ...
  const response = await api(`/api/data/orders/by_user/${auth_store.user.uuid}`)
...
const formSubmit = async () => {
  ...
  api({
    ...
    useNotify('positive','Информация обновлена')
