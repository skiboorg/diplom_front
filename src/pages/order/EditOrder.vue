    ...
    <q-form @submit="formSubmit">
      ...
      <div class="" v-if="order.service">
       ...
       <q-separator spaced="lg"/>
        <q-btn label="Добавить файл" @click="fileAction('add',null,null,null)"/>
        <q-list separator>
          <q-item v-for="(file,index) in order_files" :key="index">
            ...
            <q-item-section>
              ...
              <q-btn label="Удалить файл" :loading="loading" @click="fileAction('delete',index,order_files[index].new,file.id)"/>
            </q-item-section>
          </q-item>
        </q-list>
        <q-separator spaced="lg"/>
        <p v-if="error_text" class="text-bold text-negative">{{error_text}}</p>
        <q-btn type="submit" :loading="loading" label="Сохранить заказ"/>
